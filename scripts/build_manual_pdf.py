...
def build_story():
    ...
    story += [Spacer(1, 10 * mm), p("Modelo celular espacial para simular a resposta dos manguezais à elevação do nível do mar.", styles["CoverSub"]), Spacer(1, 10 * mm), p("Versão 1.0.0 · Windows 10/11 · 64 bits", styles["CoverMeta"]), p("Universidade Federal do Maranhão · GEOTAM", styles["CoverMeta"]), Spacer(1, 20 * mm), p("Um guia visual para preparar dados, executar cenários e interpretar resultados.", styles["CoverMeta"]), PageBreak()]

    # Presentation
    story += [PillHeading("Apresentação"), Spacer(1, 5 * mm)]
    story += [p("O BR-MANGUE Studio é um aplicativo de desktop para modelagem espacial de manguezais. Ele aplica regras de um autômato celular a uma grade raster: cada célula consulta sua elevação, seu estado e a vizinhança para calcular a transição anual.", styles["BodyBR"]), p("Este manual foi escrito para quem está começando. Você encontrará a explicação do modelo, a preparação dos arquivos, cada tela do programa, os dois motores de processamento, os arquivos de saída e uma leitura guiada de resultados reais.", styles["BodyBR"]), info_box("Importante sobre interpretação", "A simulação é uma ferramenta de cenários. O mapa mostra o comportamento das regras com os dados e parâmetros informados; não é uma previsão automática nem substitui a calibração e a validação ecológica."), Spacer(1, 5 * mm), p("A primeira versão do BR-MANGUE foi desenvolvida entre 2010 e 2014 em Lua com TerraME pelo Dr. Denilson da Silva Bezerra. O Studio reorganiza essa lógica em Python e oferece uma interface integrada para pesquisadores e usuários que não precisam programar.", styles["BodyBR"]), PageBreak()]
